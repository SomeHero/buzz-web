/**
 *      TODO: Common errors logic has a lot common, so this can be rewritten.
 */
var debug = require('debug')('buzz-web:errors');
...
module.exports = function(err, req, res, next) {
    ...
    switch (Object.getPrototypeOf(err)) {
        ...
        case NotAllowedError.prototype :
            ...
            break;
        case InvalidToken.prototype :
            res.status(401).json({ error : err.message });
            break;
        case OAuthInvalidCodeError.prototype :
            res.status(400).json({ error : err.message });
            break;
 ...
 *
 */
function OAuthInvalidCodeError(message) {
    if (! (this instanceof OAuthInvalidCodeError)) {
        return new OAuthInvalidCodeError(message);
    }

    this.name = 'OAuthInvalidCodeError';
    this.message = message;
}

OAuthInvalidCodeError.prototype = Object.create(Error.prototype);
OAuthInvalidCodeError.prototype.constructor = OAuthInvalidCodeError;

module.exports.OAuthInvalidCodeError = OAuthInvalidCodeError;